...
// ──────────────────────────────────────────────────────────────────────────
// Department Status Board — quick-glance "where is every dept right now"
// ──────────────────────────────────────────────────────────────────────────

export type DeptStatus = 'ready' | 'working' | 'blocked' | 'wrapped';

export interface DeptStatusEntry {
  dept: string;             // canonical dept name, e.g. "Camera"
  status: DeptStatus;
  note?: string;            // one-line free text
  updated_at: string;       // ISO datetime
  updated_by?: string;      // optional user tag (workspace member name, future)
}

export interface DeptStatusState {
  entries: Record<string, DeptStatusEntry>;  // keyed by canonical dept name
  custom_depts?: string[];                   // depts the user added by hand
  hidden_depts?: string[];                   // default depts the user removed
  last_updated?: string;
}
